...
def iterate_YEAR(Interest_Rate, Start_Year, End_Year, GRID_KEY,
    ...
    return Net_Benefit, Total_Benefit, Cost, Yearly_Benefit, Accumulated_Yearly_Benefit, DE_dict, BE_dict
...
def iterate_GRID(GRID_KEY_list, Interest_Rate, Start_Year, End_Year,
    ...
    for GRID_KEY in GRID_KEY_list:
        ...
        Net_Benefit, Total_Benefit, Cost, Yearly_Benefit, Accumulated_Yearly_Benefit, DE[GRID_KEY], BE[GRID_KEY] = iterate_YEAR(Interest_Rate, Start_Year, End_Year, GRID_KEY,
    ...
    return  POP_Grid_Net_Benefit, POP_Grid_Benefit, POP_Grid_Cost, Grid_Net_Benefit, Grid_Benefit, Grid_Cost, Grid_Yearly_Benefit, Grid_Accumulated_Yearly_Benefit, DE, BE
...
def Run_Model(Adaptation_Type, Start_Year = 2001, End_Year = 2010):
    ...
    POP_Grid_Net_Benefit, POP_Grid_Benefit, POP_Grid_Cost, Grid_Net_Benefit, Grid_Benefit, Grid_Cost, Yearly_Benefit, Accumulated_Yearly_Benefit, DE, BE = iterate_GRID(GRID_KEY_list, Interest_Rate, Start_Year, End_Year,
                     PM_Concentration_dict, init_ACH50_dict, phi, Adaptation_Impact, Adaptation_Cost,
                     VSL, Baseline_Mortality_dict, Relative_Risk, Population_dict)
    
    
    return  POP_Grid_Net_Benefit, POP_Grid_Benefit, POP_Grid_Cost, Grid_Net_Benefit, Grid_Benefit, Grid_Cost, Yearly_Benefit, Accumulated_Yearly_Benefit, DE, BE
...
def plot_histogram(data, title, xlabel, ylabel, color='skyblue', edgecolor='black'):
    """
    ...
    plt.show()
#%% DE Analysis
DE_Run_AQ_Grids = init_AQ_Grids()
POP_Grid = init_Population()
POP_test_1_Net, POP_test_1_Benefit, POP_test_1_Cost, test_1_Net, test_1_Benefit, test_1_Cost, a, b, de, be = Run_Model( 'LBNL_20' )
target_year = 2001
year_values_de = {key: value[target_year] for key, value in de.items() if target_year in value}
year_values_be = {key: value[target_year] for key, value in be.items() if target_year in value}

DE_Run_AQ_Grids['DE'] = DE_Run_AQ_Grids['GRID_KEY'].map(year_values_de)
DE_Run_AQ_Grids['BE'] = DE_Run_AQ_Grids['GRID_KEY'].map(year_values_be)

DE_Run_AQ_Grids['BE'] = DE_Run_AQ_Grids['BE']  * 0.7
DE_Run_AQ_Grids['DE'] = DE_Run_AQ_Grids['DE']  * 0.7
DE_Run_AQ_Grids['DM'] = ((DE_Run_AQ_Grids['GRID_KEY'].map(POP_test_1_Benefit)) / 7000000) /10
DE_Run_AQ_Grids['Pop_Benefit'] = ((DE_Run_AQ_Grids['GRID_KEY'].map(POP_test_1_Benefit)) / 1000000) / 10
DE_Run_AQ_Grids['POP'] = DE_Run_AQ_Grids['GRID_KEY'].map(POP_Grid)

PM_out = init_PM_Concentrations_means()
DE_Run_AQ_Grids['PM'] = DE_Run_AQ_Grids['GRID_KEY'].map(PM_out['2001'])

Total_Pop = DE_Run_AQ_Grids['POP'].sum()
Total_Benefit_Billion = DE_Run_AQ_Grids['Pop_Benefit'].sum() / 1000
Total_Mortality = DE_Run_AQ_Grids['DM'].sum()

Average_PM = 0
Average_DE = 0
Average_BE = 0
for idx, row in DE_Run_AQ_Grids.iterrows():
    Average_DE += (row['DE'] * row['POP'])/Total_Pop
    Average_BE += (row['BE'] * row['POP'])/Total_Pop
    Average_PM += (row['PM'] * row['POP'])/Total_Pop
    

    
plot_geodataframe(gdf = DE_Run_AQ_Grids, title = 'Delta Indoor Exposure 2001', column_to_plot = 'DE', legend_tag = 'ug/m3')
plot_geodataframe(gdf = DE_Run_AQ_Grids, title = 'Indoor Exposure due to Outdoor Origin 2001', column_to_plot = 'BE', legend_tag = 'ug/m3')

plot_geodataframe(gdf = DE_Run_AQ_Grids, title = 'Average Annual Delta Mortality 2001-2010', column_to_plot = 'DM', legend_tag = 'People')
plot_geodataframe(gdf = DE_Run_AQ_Grids, title = 'Average Annual Total Benefit 2001-2010', column_to_plot = 'Pop_Benefit', legend_tag = 'Million Dollar')

plot_geodataframe(gdf = DE_Run_AQ_Grids, title = 'Ambient PM 2001', column_to_plot = 'PM', legend_tag = 'concentration')

#%% Finf single detached

ach_50_dict = init_ACH50_mean()
ach_50_dict = ach_50_dict['Single-Family Detached']
Finf_dict = {}
for g in ach_50_dict.keys():
    Finf_dict[g] = ACH50_to_INF(ach_50_dict[g])
    
ACH_Run_AQ_Grids = init_AQ_Grids()
ACH_Run_AQ_Grids['ach'] = ACH_Run_AQ_Grids['GRID_KEY'].map(ach_50_dict)
ACH_Run_AQ_Grids['finf'] = ACH_Run_AQ_Grids['GRID_KEY'].map(Finf_dict)

plot_geodataframe(gdf = ACH_Run_AQ_Grids, title = 'Single Family Detached ACH50', column_to_plot = 'ach', legend_tag = 'ach50')
plot_geodataframe(gdf = ACH_Run_AQ_Grids, title = 'Single Family Detached Finf', column_to_plot = 'finf', legend_tag = '')

ACH_Run_AQ_Grids['finf'].hist()
plot_histogram(ACH_Run_AQ_Grids['finf'], 'Single Family Detached Finf', 'Finf', 'frequency')
